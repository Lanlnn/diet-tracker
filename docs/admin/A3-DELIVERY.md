...
- 后端 H2：OpenJDK 17.0.19，48 项测试 0 failure、0 error；仅跳过 2 个显式外部 MySQL/性能用例。
- 后端真实 MySQL 8.4.10：从干净 `target/` 运行 48 项测试，0 failure、0 error；迁移测试实际执行，只有 API 性能门禁按环境条件跳过。
...
- 隔离 MySQL 8.4.10 空库成功校验并执行 10 个迁移，最终版本 V10，并通过 Hibernate `ddl-auto=validate`。
- MySQL 集成测试验证最终版本 V10、基础食品、M10 表结构及 supportRef 格式/非空/唯一性，1/1 通过。
...
- 整理后的 `codex/a3-support-diagnostics` 直接推送交付；旧 `codex/a2-admin-foods` 仅保留历史参考，不单独合并。
