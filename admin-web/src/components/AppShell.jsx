...
const nav = [
  ...
  { to: '/user-diagnostics', label: '用户诊断', icon: 'user', roles: supportRoles },
  { to: '/security', label: '系统与安全', icon: 'shield' },
]
const titles = { '/': '阶段总览', '/foods': '系统食品', '/categories': '食品分类', '/custom-foods': '私有食品诊断', '/user-diagnostics': '用户诊断', '/security': '系统与安全' }
...
export function AppShell() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      ...
      <nav aria-label="主导航"><p>当前交付 · A3</p>{allowed.map((item) => <NavLink key={item.to} to={item.to} end={item.to === '/'} onClick={() => setOpen(false)}><Icon name={item.icon}/><span>{item.label}</span></NavLink>)}</nav>
    ...
    <div className="main-shell">
      <header className="topbar"><button className="icon-button menu" type="button" aria-label="打开导航" onClick={() => setOpen((v) => !v)}><Icon name="menu"/></button><span>管理后台</span><Icon name="chevron" size={14}/><strong>{titles[location.pathname] || '页面'}</strong><div className="env"><i/>A3 只读诊断</div></header>
      <main><Outlet /></main>
    </div>
